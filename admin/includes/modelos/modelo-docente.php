...
if( $accion === "crear"){
    ...
    try {
        ...
        $stmt = $conn -> prepare("INSERT INTO DOCENTE (dni, nombres, apellidos, tipoUsuario, especialidad, estado, usuarioCreacion, horaCreacion,
        ...
        $stmt -> bind_param("sssisiss", $dni, $nombres, $apellidos, $tipoUsuario, $especialidad, $estado, $usuarioSesion, $usuarioSesion);
        $stmt -> execute();
...
if($accion === "actualizar"){
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE DOCENTE SET dni = ?, nombres = ?, apellidos = ?, tipoUsuario = ?, especialidad = ?, estado = ?, usuarioModificacion = ?, horaModificacion = CURRENT_TIME(), fechaModificacion = CURRENT_DATE() WHERE id = ? ");

        $stmt -> bind_param("sssisisi", $dni, $nombres, $apellidos, $tipoUsuario, $especialidad, $estado, $usuarioSesion, $id);
        $stmt -> execute();
